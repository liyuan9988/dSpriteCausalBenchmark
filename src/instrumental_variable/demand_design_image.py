...
from ..instrumental_variable.demand_design import generate_test_demand_design, generate_demand_design_iv
...
def generate_test_demand_design_image(org_test: IVTestDataSet) -> IVTestDataSet:
    """
    Returns
    -------
    ...
    return IVTestDataSet(treatment=treatment,
                         covariate=covariate_img,
                         structural=target)


def generate_demand_design_image_iv(data_size: int,
                                    rho: float,
                                    rand_seed: int = 42):
    """
    Parameters
    ...
    """
    org_train, org_test = generate_demand_design_iv(data_size, rho, rand_seed)
    ...
    train_data = IVTrainDataSet(treatment=org_train.treatment,
                                instrumental=instrument_img,
                                covariate=covariate_img,
                                outcome=org_train.outcome,
                                structural=org_train.structural)
    test_data = generate_test_demand_design_image(org_test)
